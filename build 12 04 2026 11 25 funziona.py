...
if os.path.exists(FIP_DATA_FILE):
    try:
        ...
        json.loads(fip_json)  # verifica JSON valido
...
# Sostituisce il placeholder ovunque appaia (pattern typeof o semplice)
# Pattern: const FIP_SARDA = (typeof __FIP_SARDA_JSON__ !== 'undefined') ? __FIP_SARDA_JSON__ : {...};
# oppure:  const FIP_SARDA = __FIP_SARDA_JSON__;
template = re.sub(
    r'const FIP_SARDA\s*=\s*(?:\(typeof __FIP_SARDA_JSON__ !== \'undefined\'\)\s*\?\s*__FIP_SARDA_JSON__\s*:\s*\{[^}]*\}|__FIP_SARDA_JSON__)\s*;',
    f'const FIP_SARDA = {fip_json};',
    template,
    count=1
)
# ───────────────────────────────────────────────────────────────
