...
FASI_ELIM = {
    'quarti', 'semifinal', 'finale', 'final', 'playout', 'play-out',
    'play out', 'ottavi', 'turno', '1° turno', '2° turno',
}

def is_fase_elim(fase_nome):
    """True se la fase è eliminazione diretta (no andata/ritorno, gare secche)."""
    if not fase_nome:
        return False
    fn = fase_nome.lower()
    return any(k in fn for k in FASI_ELIM)


def scrape_leg(sesso, camp_code, fase_code, girone_code, codice_ar, leg_len):
    """Scarica andata (codice_ar=1) o ritorno (codice_ar=0) separatamente."""
...
def scrape_girone_elim(sesso, camp_code, fase_code, girone_code, girone_nome):
    """Scarica partite di una fase a eliminazione diretta (gare secche, no codice_ar).
    Itera le giornate senza split andata/ritorno, fino a 10 gare max.
    """
    base_kw = dict(sesso=sesso, codice_campionato=camp_code,
                   codice_fase=fase_code or None,
                   codice_girone=girone_code or None)
    ris, pro = [], []
    vuote = 0
    for g in range(1, 15):  # max 10 gare + 4 buffer
        soup = fetch(build_url(**base_kw, giornata=g))
        time.sleep(SLEEP)
        if not soup:
            vuote += 1
        ...
        vuote = 0
        for e in r + p:
            e['gara'] = g  # Gara 1, Gara 2, Gara 3...
        ris.extend(r)
        pro.extend(p)
    log(f"       '{girone_nome}' [ELIM]: {len(ris)}r+{len(pro)}p")
    return ris, pro


def scrape_girone(sesso, camp_code, fase_code, girone_code, girone_nome, classifica, fase_nome=''):
    """Scarica TUTTE le partite di un girone.
    - Fasi normali (qualificazione, ecc.): usa codice_ar=1 e 0 separatamente.
    - Fasi eliminazione diretta (quarti, semifinali, finale, playout turni):
      scarica senza codice_ar, tratta le giornate come Gara 1/2/3.
    """
    if is_fase_elim(fase_nome) or is_fase_elim(girone_nome):
        ris, pro = scrape_girone_elim(sesso, camp_code, fase_code, girone_code, girone_nome)
        return {
            'nome':       girone_nome,
            'fase':       fase_nome,
            'elim':       True,
...
def scrape_campionato(sesso, camp_code, camp_nome):
    ...
    # Cerca fasi aggiuntive aperte in corso di stagione:
    # carica ogni fase e controlla se nel suo select appaiono fasi nuove
    fasi_viste = set(f[0] for f in fase_opts)
    for fase_code, _ in list(fase_opts):
        if not fase_code:
            continue
        ...
        if not sf:
            continue
        nuove = select_opts(sf, 3)
        for fc, fn in nuove:
            if fc and fc not in fasi_viste:
                log(f"     ++ Nuova fase scoperta: {fc!r} '{fn}'")
                fasi_viste.add(fc)
                fase_opts.append((fc, fn))
    ...
    for fase_code, fase_nome in fase_opts:
        ...
        for girone_code, girone_nome in girone_opts:
            ...
            g_data = scrape_girone(sesso, camp_code, fase_code, girone_code, full_nome, cl_g, fase_nome=fase_nome)
