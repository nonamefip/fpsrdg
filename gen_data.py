...
for camp in all_camp_names:
    ...
    # Gironi semplici per compatibilità template esistente
    gironi=sorted(set(g.get('Girone','') for g in cg if g.get('Girone','')))
    # Gironi con fase: tiene separate fasi omonime (es. Playout vs Qualificazione con stesso nome girone)
    _gf_seen={}
    for g in cg:
        _f=g.get('Fase',''); _gi=g.get('Girone','')
        if _gi: _gf_seen[(_f,_gi)]=True
    gironi_con_fase=[{'fase':k[0],'girone':k[1]} for k in sorted(_gf_seen.keys())]
    ...
    camp_stats[camp]={
        ...
        'gironi_con_fase':gironi_con_fase,
